/* games/flood-fill/game.js — 패턴 D (퍼즐 병렬 경쟁): 색깔 채우기 (flood fill) */
'use strict';
...
let roundResults = [];
let zoneStates = [];             // each player's current GRID_SIZE*GRID_SIZE color-index array
let zoneMoves = [];
let zoneSolved = [];
let phase = 'idle';
...
let initialGridForRound = null;  // shared starting grid for all players this round
...
function isSolved(state) {
  ...
  for (let i = 1; i < state.length; i++) {
    if (state[i] !== first) return false;
  }
  return true;
}

// flood fill from index 0, replacing the connected region (matching the
// CURRENT top-left color) with newColorIdx
function floodFill(state, newColorIdx) {
  const oldColor = state[0];
  if (oldColor === newColorIdx) return state.slice();
  ...
  while (stack.length) {
    const idx = stack.pop();
    next[idx] = newColorIdx;
    const r = Math.floor(idx / GRID_SIZE), c = idx % GRID_SIZE;
    const neighbors = [];
    if (r > 0) neighbors.push(idx - GRID_SIZE);
    if (r < GRID_SIZE - 1) neighbors.push(idx + GRID_SIZE);
    if (c > 0) neighbors.push(idx - 1);
    if (c < GRID_SIZE - 1) neighbors.push(idx + 1);
    neighbors.forEach((n) => {
      if (!visited[n] && state[n] === oldColor) {
        visited[n] = true;
        stack.push(n);
      }
    });
...
// ── Build zones (each zone has its own grid + color swatches) ──
function buildZones() {
  zonesWrap.innerHTML = '';
  ...
  for (let i = 0; i < playerCount; i++) {
    ...
    grid.className = 'puzzle-grid flood-grid';
    grid.id = `puzzle-grid-${i}`;
    zone.appendChild(grid);

    const swatchRow = document.createElement('div');
    swatchRow.className = 'flood-swatch-row';
    swatchRow.id = `swatch-row-${i}`;
    COLORS.forEach((color, colorIdx) => {
      const btn = document.createElement('button');
      btn.className = 'flood-swatch';
      btn.style.background = color;
      btn.dataset.player = i;
      btn.dataset.color = colorIdx;
      onTap(btn, () => handleSwatchTap(i, colorIdx));
      swatchRow.appendChild(btn);
    });
    zone.appendChild(swatchRow);
...
function renderPuzzle(playerIdx) {
  ...
  state.forEach((colorIdx) => {
    const cell = document.createElement('div');
    cell.className = 'puzzle-tile flood-cell';
...
// ── Swatch tap handler ───────────────────────────────────────
function handleSwatchTap(playerIdx, colorIdx) {
  if (phase !== 'active') return;
  if (zoneSolved[playerIdx]) return;

  zoneStates[playerIdx] = floodFill(zoneStates[playerIdx], colorIdx);
  zoneMoves[playerIdx]++;
  sound.play('slide');
...
function loadRound() {
  ...
  initialGridForRound = makeRandomGrid();
  zoneStates = [];
  zoneMoves = [];
  zoneSolved = [];

  for (let i = 0; i < playerCount; i++) {
    zoneStates.push(initialGridForRound.slice());
    zoneMoves.push(0);
  ...
  problemStatus.textContent = '색을 눌러 한 가지 색으로 통일하세요!';
...
// ── Result ───────────────────────────────────────────────────
function showResult() {
  var scoreResult = reportGameResult({ gameId: 'flood-fill', playerCount: playerCount, scores: scores.slice(), metric: 'score' });
  (function () {
