/* games/times-table-ladder/game.js */
...
// ── Problem generation ───────────────────────────────────────
const EASY_DAN = [2, 5, 10];

function generateQuestion() {
  const dan = difficulty === 'easy' ? EASY_DAN[rand(0, EASY_DAN.length - 1)] : rand(2, 9);
  const startB = rand(1, 7); // startB, startB+1, startB+2 (blank) all within 1..9

  const row1 = `${dan}×${startB} = ${dan * startB}`;
  const row2 = `${dan}×${startB + 1} = ${dan * (startB + 1)}`;
  const answer = dan * (startB + 2);
  const expr = `${row1}, ${row2}, ${dan}×${startB + 2} = ?`;

  // Generate 3 wrong answers: nearby multiples of dan (plausible mistakes), no dups, no negatives
  const wrongSet = new Set();
  while (wrongSet.size < 3) {
    const offset = (rand(1, 3) * dan) * (Math.random() < 0.5 ? 1 : -1);
    const w = answer + (offset === 0 ? dan : offset);
...
// ── Result screen ─────────────────────────────────────────────
function showResult() {
  var scoreResult = reportGameResult({ gameId: 'times-table-ladder', playerCount: playerCount, scores: scores.slice(), metric: 'score' });
  (function () {
