...
(function () {
  ...
  function nextTarget(playerIdx) {
    var dan = difficulty === 'easy' ? EASY_DAN[rand(0, EASY_DAN.length - 1)] : rand(2, 9);
    var b   = rand(1, 9);
    zoneTargets[playerIdx] = { dan: dan, b: b, answer: dan * b };
    var label = targetLabels[playerIdx];
    if (label) label.textContent = dan + ' × ' + b + ' = ?';
  }
  ...
  // ===== 결과 화면 =====
  function showResult() {
    var scoreResult = reportGameResult({ gameId: 'times-table-pop', playerCount: playerCount, scores: playerScores.slice(), metric: 'score' });
