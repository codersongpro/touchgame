/* games/times-table-quiz/game.js */
...
function generateQuestion() {
  let dan, b, answer, expr;

  if (difficulty === 'easy') {
    // 저학년 친화: 2단·5단·10단만 출제
    dan = EASY_DAN[rand(0, EASY_DAN.length - 1)];
    b   = rand(1, 9);
  } else {
    // 보통: 2단~9단 전체
    dan = rand(2, 9);
  ...
  // Generate 3 wrong answers: correct ± dan 배수 오프셋, no dups, no negatives
  const wrongSet = new Set();
  while (wrongSet.size < 3) {
    const offset = (rand(1, 4) * dan) * (Math.random() < 0.5 ? 1 : -1);
...
// ── Result screen ─────────────────────────────────────────────
function showResult() {
  var scoreResult = reportGameResult({ gameId: 'times-table-quiz', playerCount: playerCount, scores: scores.slice(), metric: 'score' });
  (function () {
